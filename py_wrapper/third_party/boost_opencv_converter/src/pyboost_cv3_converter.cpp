...
PyObject* matDoubleToNDArrayBoostConverter::convert(const Mat_<double> &m) {
	if (!m.data)
		Py_RETURN_NONE;
		Mat temp,
	*p = (Mat*) &m;
...
matDoubleFromNDArrayBoostConverter::matDoubleFromNDArrayBoostConverter() {
	boost::python::converter::registry::push_back(convertible, construct,
			boost::python::type_id<Mat_<double> >());
}

/// @brief Check if PyObject is an array and can be converted to OpenCV matrix.
void* matDoubleFromNDArrayBoostConverter::convertible(PyObject* object) {
	if (!PyArray_Check(object)) {
		return NULL;
	}
#ifndef CV_MAX_DIM
...
/// @brief Construct a Mat from an NDArray object.
void matDoubleFromNDArrayBoostConverter::construct(PyObject* object,
		boost::python::converter::rvalue_from_python_stage1_data* data) {
	namespace python = boost::python;
	// Object is a borrowed reference, so create a handle indicting it is
	// borrowed for proper reference counting.
	...
	// Obtain a handle to the memory block that the converter has allocated
	// for the C++ type.
	typedef python::converter::rvalue_from_python_storage<Mat_<double> > storage_type;
	void* storage = reinterpret_cast<storage_type*>(data)->storage.bytes;
