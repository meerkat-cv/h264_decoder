...
struct matDoubleToNDArrayBoostConverter {
	static PyObject* convert(Mat_<double> const& m);
};
...
struct matDoubleFromNDArrayBoostConverter {

	matDoubleFromNDArrayBoostConverter();

	/// @brief Check if PyObject is an array and can be converted to OpenCV matrix.
	static void* convertible(PyObject* object);

	/// @brief Construct a Mat_<double> from an NDArray object.
	static void construct(PyObject* object,
			boost::python::converter::rvalue_from_python_stage1_data* data);
};
